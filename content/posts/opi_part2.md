...
```
In my case, even setting INSTALL_HDR_PATH didn't guarantee that the headers would be installed properly. In that case, I copied the local header files that are generated by the above command into /mnt/usr/ manually as follows:
```
